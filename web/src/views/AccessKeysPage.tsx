...
import { Button, Card, DatePicker, Empty, Form, Input, Modal, Popconfirm, Skeleton, Space, Switch, Table, Tag, Typography, message } from 'antd'
  ...
  return (
    <div className="mc-enter mx-auto max-w-[1500px]">
      ...
      <div className="space-y-3 lg:hidden">
        {keys.isPending ? <Card><Skeleton active paragraph={{ rows: 3 }} /></Card> : keys.data?.length ? keys.data.map((key) => (
          <Card key={key.id} size="small">
            <div className="mb-3 flex items-start justify-between gap-3">
              ...
              <Tag className="shrink-0" color={key.enabled ? 'success' : undefined}>{key.enabled ? '启用' : '停用'}</Tag>
            </div>
        ...
        )) : <Card><Empty className="py-8" description="暂无访问密钥" /></Card>}
      </div>
