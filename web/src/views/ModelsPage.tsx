...
export default function ModelsPage() {
  ...
  return (
    <div className="mc-enter mx-auto max-w-[1500px]">
      <div className="mb-8 flex items-end justify-between"><Typography.Title level={2} className="!mb-0 !tracking-[-.04em]">模型路由</Typography.Title><Button type="primary" icon={<PlusOutlined />} onClick={createModel} disabled={!providers.data?.length}>新增虚拟模型</Button></div>
      <Table rowKey="id" loading={models.isPending} dataSource={models.data ?? []} pagination={false} expandable={{ expandedRowRender: (model) => <div className="p-4">{model.candidates.map((candidate) => <div key={candidate.id} className="mb-3 grid grid-cols-[44px_1fr_1fr_2fr] items-center gap-4 border-b border-black/5 pb-3 dark:border-white/5"><span className="font-mono text-[#d7783d]">{String(candidate.position + 1).padStart(2, '0')}</span><strong>{candidate.provider_name}</strong><code>{candidate.upstream_model}</code><Space wrap>{candidate.protocols.map((item) => <Tag key={item.protocol}>{protocolLabels[item.protocol]}</Tag>)}</Space></div>)}</div> }} columns={[
      ...
      <Modal title={editing ? '编辑模型路由' : '新增虚拟模型'} width={820} open={open} onCancel={() => { setOpen(false); setEditing(null) }} onOk={() => form.submit()} confirmLoading={save.isPending} okText="保存路由">
...
function ProtocolOrderField({ value = [], onChange, available }: { value?: CandidateProtocol['protocol'][]; onChange?: (value: CandidateProtocol['protocol'][]) => void; available: CandidateProtocol['protocol'][] }) {
  ...
  return <div className="space-y-2 rounded border border-black/10 bg-black/[.015] p-3 dark:border-white/10 dark:bg-white/[.02]">
    {value.map((protocol, index) => <div key={`${protocol}-${index}`} className="grid grid-cols-[34px_1fr_32px_32px_32px] items-center gap-2">
