...
- 模型路由支持在管理后台直接测试虚拟模型连通性；管理后台支持修改管理员密码和退出登录。
...
推送符合 `vX.Y.Z` 格式的标签后，[发布工作流](.github/workflows/release.yml)会构建前端、运行 Go 测试、交叉编译三个平台的二进制、生成 SHA-256 校验文件、发布 `linux/amd64`、`linux/arm64` Docker 镜像，并创建 GitHub Release。
